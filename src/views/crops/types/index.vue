  ...
  <div class="ele-body">
    ...
    <el-card shadow="never">
      ...
      >
        ...
        <template slot="action" slot-scope="{ row }">
          <el-link
            v-permission="'crop_type_up'"
            @click="updateCropType(row)"
            type="success"
            :underline="false"
            >修改
          </el-link>
          <el-popconfirm
            v-permission="'crop_type_del'"
            class="ele-action"
            title="确定要删除吗？"
    ...
    >
      <el-form
        ...
        @submit.native.prevent
      >
        <el-form-item label="作物类型:" prop="name">
          <el-input
            v-model="form.name"
            placeholder="请输入作物类型"
            clearable
...
export default {
  data() {
    return {
      ...
      // 编辑表单验证规则
      rules: {
        name: [{ required: true, message: "请输入作物类型", trigger: "blur" }]
      },
      loading: false,
      edit: false,
      updateId: null
        ...
        if (valid) {
          ...
          try {
            if (this.dialogType === 1) {
              await PostCropTypesSave(this.form);
            } else {
              await PutCropTypesUpdate(this.updateId, this.form);
            }
