    ...
    <el-card shadow="never">
      ...
      >
        ...
        <template slot="toolbar">
          <el-button
            v-permission="'question_type_add'"
            type="primary"
            icon="el-icon-plus"
            class="ele-btn-icon"
        ...
        <template slot="action" slot-scope="{ row }">
          <el-link
            v-permission="'question_type_up'"
            @click="updateQuestionType(row)"
            type="success"
            :underline="false"
          >修改
          </el-link>
          <el-popconfirm
            v-permission="'question_type_del'"
            class="ele-action"
            title="确定要删除吗？"
            @confirm="deleteQuestionType(row)"
          >
            <el-link
              type="danger"
              slot="reference"
...
<script>
import {
  postQuestionTypeSave,
  deleteQuestionType,
  putQuestionTypeUpdate
} from '@/api/questionType';
  ...
  data() {
    return {
      ...
      rules: {
        ...
        icon:[{ required: true, message: '请选择类型图标', trigger: 'change' }]
      },
      loading: false,
      edit: false,
      updateId:null
        ...
        if (valid) {
          ...
          try {
            if(this.dialogType===1){
              await postQuestionTypeSave(this.form);
            }else {
              await putQuestionTypeUpdate(this.updateId,this.form);
            }
            this.$message.success(this.dialogType  === 1 ? '添加成功' : '修改成功');
            this.edit = false
            this.reload();
            await this.$store.dispatch('setQuestionType');
            this.loading = false
          } catch {
            this.loading = false
          }
