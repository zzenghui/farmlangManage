    ...
    <el-card shadow="never">
      ...
      >
        ...
        <template slot="action" slot-scope="{ row }">
          ...
          <el-popconfirm
            v-permission="'product_type_del'"
            class="ele-action"
            title="确定要删除吗？"
            @confirm="deleteProductType(row)"
          >
...
import {
  PostProductTypeSave,
  PutProductTypeUpdate,
  DeleteProductType
} from "@/api/productType/index.js";
export default {
  data() {
    return {
        ...
        if (valid) {
          ...
          try {
            if (this.dialogType === 1) {
              await PostProductTypeSave(this.form);
            } else {
              await PutProductTypeUpdate(this.updateId, this.form);
            }
