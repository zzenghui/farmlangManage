          ...
          <div class="box-house">
            ...
            >
              ...
              >
                ...
                >
                  <el-option
                    v-for="(arr,index) in questionGradeData"
                    :key="index"
                    :label="arr.name"
                    :value="arr.id"
                  >
...
<!--              </el-form-item>-->
      ...
      <el-col :lg="8" :md="24">
        <user-field-map :serviceData="true" @changeService="getRecordData"></user-field-map>
      </el-col>
    </el-row>
  </div>
</template>
...
import { postQuestionRecordSave } from "@/api/questionRecord";
...
//       this.remarks=obj;
    ...
    imgSaveToUrl(file){
      this.num++;
      if(this.uploadImgData.length > 8&& this.num===1){
  ...
  async mySave(){
     ...
     if(this.uploadImgData.length > 0){
       for (let file in this.formData){
         // new Compressor(this.formData[file],{
         //   quality:0.4,
         //   success:async (f)=>{
             let fromData=new FormData();
             fromData.append('file',this.formData[file]);
             const { data } = await PostProductImageUpload(fromData);
             images.push({path:data[0].name,thumbPath:data[1].name});
         //   }
         // })
       }
     }
     this.form.album=images;
    },
    //过滤方案数据
    filterSolution() {
